...
import {MixedProduct, Order, OrderWithDetailsRequest} from './order.model';
...
export class OrderService {
    ...
    getMixedProducts(clientId: string): Observable<MixedProduct[]> {
        return this.getHeaders().pipe(
            switchMap(headers =>
                this.http.get<MixedProduct[]>(`${this.baseUrl}/${clientId}`, {
                    headers,
                    withCredentials: true
                })
            ),
            catchError(err => {
                console.error('Erreur récupération mixed products :', err);
