...
import {Product, ProducttService} from "../products/productt.service";
...
export class OrderesclientComponent implements OnInit {
    ...
    constructor(private orderService: OrderService,private productService: ProducttService,private billingService: BillingserviceService,private router: Router) {}
    typeOptions = [
    ...
    generateInvoice(order: any) {
        this.billingService.generateInvoice(order.id).subscribe({
            next: (res) => {
                console.log('Facture générée avec succès', res);
                alert('Facture générée !');
                this.router.navigate(['/client/billing']);
            },
            error: (err) => {
